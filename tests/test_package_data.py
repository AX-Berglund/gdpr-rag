...
class TestDemoResilience:
    ...
    def test_examples_fall_back_when_the_set_is_unreadable(self, demo_namespace):
        def boom():
            raise FileNotFoundError("no evaluation set")

        demo_namespace["load_questions"] = boom
        examples = demo_namespace["_examples"]()
        assert examples == demo_namespace["FALLBACK_EXAMPLES"]
